...
- **`1_Linear_Regression_G3.ipynb`**: Xây dựng mô hình Hồi quy tuyến tính (Linear Regression) để dự đoán điểm số cuối kỳ `G3`.
...
- **`3_Clustering_KMeans.ipynb`**: Phân nhóm học sinh (Segmentation) bằng thuật toán học không giám sát K-Means Clustering và tìm kiếm cụm tối ưu qua phương pháp Elbow.
- **`4_Dimension_Reduction_PCA.ipynb`**: Giảm chiều dữ liệu (Dimensionality Reduction) từ hơn 30 đặc trưng xuống không gian 2D bằng thuật toán PCA.
...
- **`6_Model_Interpretation_XAI.ipynb`**: Sử dụng thư viện SHAP (Explainable AI) để "mở hộp đen" mô hình, giải thích chi tiết mức độ đóng góp của từng thói quen/hoàn cảnh đến điểm số của học sinh.
- **`7_Ethical_Bias_Fairness.ipynb`**: Đánh giá tính đạo đức và sự công bằng (Fairness) của mô hình AI đối với các nhóm nhạy cảm (như Giới tính) bằng Fairlearn.
